...
export default function Covering() {
  ...
  const llantaHandler = (value) => {
    if (value) {
      dispatch({ type: types.addLlanta });
    } else {
      dispatch({ type: types.rmLlanta });
    }
  };

  const choqueHandler = (value) => {
    if (value) {
      dispatch({ type: types.addChoque });
    } else {
      dispatch({ type: types.rmChoque });
    }
  };

  const atropelloHandler = (value) => {
    if (value) {
      dispatch({ type: types.addAtropello });
    } else {
      dispatch({ type: types.rmAtropello });
    }
  };
          ...
          >
                    ...
                    <Row>
                      ...
                      <Col xs={3}>
                        <label className="switch">
                          <input
                            type="checkbox"
                            onChange={(ev) => llantaHandler(ev.target.checked)}
                          />
                    ...
                    <Row>
                      ...
                      <Col xs={3}>
                        <label className="switch">
                          <input
                            type="checkbox"
                            onChange={(ev) => choqueHandler(ev.target.checked)}
                          />
                          <span className="slider round"></span>
                        </label>
                      </Col>
                    ...
                    <Row>
                      ...
                      <Col xs={3}>
                        <label className="switch">
                          <input
                            type="checkbox"
                            onChange={(ev) => atropelloHandler(ev.target.checked)}
                          />
                          <span className="slider round"></span>
                        </label>
                      </Col>
            ...
            <Tab eventKey="profile" title="Protege a los que te rodean">
              <p style={{ paddingLeft: 32, paddingRight: 32 }}>
                ...
                repellendus, voluptatum nobis earum exercitationem consequuntur?
                Laboriosam ut tenetur quas officiis commodi minus animi aliquam.
              </p>
            </Tab>
